...
const timberData = {
  ...
  "Mysore Honne": {
    description:
      "Mysore teak is highly regarded for its unique characteristics and is sourced from the Mysore region in Karnataka, India. It features a beautiful golden hue and excellent workability, making it suitable for premium furniture and interior wood paneling.",
    images: [
          ...
          <div className="max-w-6xl mx-auto px-6 py-10">
            <div className="grid md:grid-cols-5 gap-8">
              ...
              <div className="md:col-span-3 order-2 md:order-1">
                {currentData.images ? (
                  <>
                    <div className="relative rounded-xl overflow-hidden aspect-video bg-[#F0E5CF] dark:bg-[#3B2C1A] shadow-inner mb-4">
  {currentData.images?.map((img, i) => (
    <MOTION.div
      key={i}
      className="absolute inset-0"
      initial={{ opacity: 0 }}
      animate={{ opacity: i === activeImageIndex ? 1 : 0 }}
      transition={{ duration: 0.5 }}
      style={{ pointerEvents: i === activeImageIndex ? 'auto' : 'none' }}
    >
      {i === activeImageIndex && (
        <ZoomViewer key={i} src={img} zoom={2.5} />
      )}
    </MOTION.div>
  ))}
</div>
