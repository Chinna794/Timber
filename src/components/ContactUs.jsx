...
//       const marker = window.L.marker(activeBranch).addTo(mapRef.current);
//       marker.bindPopup(`Location: ${activeBranch.name}`);
//       markerRef.current = marker;
//       mapRef.current.setView(activeBranch, 14);
//     }
//   }, [activeBranch]);

//   return (
//     <div id="contact" className={`flex flex-col md:flex-row w-full min-h-screen ${luxury.bg} ${luxury.textMain}`}>
//       <div className="w-full md:w-1/2 p-6 sm:p-8 md:p-12 flex flex-col justify-center">
//         <h1 className={`text-2xl sm:text-3xl md:text-4xl font-serif font-bold mb-2 ${luxury.heading}`}>Get in touch</h1>
//         <p className={`${luxury.textSub} mb-6 `}>We're here to help. Chat to our team 24/7 and get started in minutes.</p>
...
//           <div className="mb-6">
//   <label className={`block text-sm font-medium mb-2 ${luxury.textLabel}`}>Comments or Questions</label>
//   <textarea
//     name="comments"
...
//     <button
//       key={index}
//       onClick={() => setActiveBranch(branch.coords)}
//       className={`px-3 py-1 rounded shadow text-sm transition ${luxury.tabInactive}`}>
//       {branch.name}
//     </button>
//   ))}
...
const ContactUs = () => {
  ...
  const validateForm = () => {
    const hasSelectedTimber = Object.values(formData.services).some(value => value);
...
/>
          ...
          <div className="mb-6">
            <label className={`block text-sm font-medium mb-2 ${luxury.textLabel}`}>
            ...
            {validationErrors.services && (
              <p className="text-red-500 text-sm mt-1">Please select at least one timber option</p>
            )}
          </div>
  ...
  rows={4}
  placeholder="Tell us more about what you're looking for..."
  className={`w-full p-2 border ${validationErrors.comments ? 'border-red-500' : 'border-gray-300'} rounded bg-white focus:ring-2 focus:ring-yellow-600 text-gray-800`}
/>
            {validationErrors.comments && (
              <p className="text-red-500 text-sm mt-1">Please provide your comments or questions</p>
            )}
          </div>
